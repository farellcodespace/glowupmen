...
function bdLatestMood(){
  const dates=Object.keys(journal||{}).sort();
  for(let i=dates.length-1;i>=0;i--){ const m=journal[dates[i]].mood; if(m) return {e:jMoodEmoji(m), d:dates[i]}; }
  return null;
}
function bdFinance(){
  const mk=todayStr().slice(0,7);
  const tx=(finance||[]).filter(t=>(t.date||'').slice(0,7)===mk);
  const income=tx.filter(t=>t.type==='in').reduce((s,t)=>s+(+t.amount||0),0);
  const expense=tx.filter(t=>t.type==='out').reduce((s,t)=>s+(+t.amount||0),0);
  if(financeBudget>0) return {v:fmtRp(financeBudget-expense), l:'sisa budget bln ini'};
  if(tx.length) return {v:fmtRp(income-expense), l:'saldo bln ini'};
  return null;
}
function bdGreeting(){
...
function renderBeranda(){
  ...
  html+=`<div class="bd-stats">
    ...
    ${(()=>{const m=bdLatestMood();return m?stat('smile',m.e,'mood · '+fmtShort(m.d)):stat('smile','Mood <span class="bd-tag-soon">segera</span>','Jurnal','soon');})()}
  </div>`;
  html+=`<div class="bd-stats" style="margin-top:-8px;">
    ${(()=>{const f=bdFinance();return f?stat('banknote','<span style="font-size:14px;">'+f.v+'</span>',f.l):stat('banknote','Budget <span class="bd-tag-soon">segera</span>','Keuangan','soon');})()}
  </div>`;
