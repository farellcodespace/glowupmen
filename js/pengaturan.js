...
function exportData(){
  ...
  const payload={
    _app:'glowupmen', _v:4, _exported:d.toISOString(),
    ...
    gum_journal:journal,      // jurnal harian (v3)
    gum_finance:finance,            // transaksi keuangan (v4)
    gum_finance_budget:financeBudget, // budget pengeluaran/bulan (v4)
  };
...
function importData(file){
  ...
  reader.onload=()=>{
    ...
    if(d.gum_journal && typeof d.gum_journal==='object' && !Array.isArray(d.gum_journal)){ journal=d.gum_journal; LS.set('gum_journal',journal); } // v3
    if(Array.isArray(d.gum_finance)){ finance=d.gum_finance; LS.set('gum_finance',finance); } // v4
    if(typeof d.gum_finance_budget==='number'){ financeBudget=d.gum_finance_budget; LS.set('gum_finance_budget',financeBudget); } // v4
    applyTheme();
